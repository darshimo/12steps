...
typedef struct _kz_msgbox {
    ...
    /* H8は16ビットCPUなので、32ビット整数に対しての乗算命令がない。よって
       ...
       計算で乗算が使われて、「__mulsi3がない」などのリンクエラーになる場合が
       ある。（2の累乗ならばシフト演算が利用されるので問題は出ない）
       対策として、サイズが2の累乗になるようにダミーメンバで調節する。
...
static kz_thread threads[THREAD_NUM]; /* タスクコントロールボロック */
...
/* システムコールの処理(kz_wait() : スレッドの実行けん放棄) */
...
static void srvcall_proc(kz_syscall_type_t type, kz_syscall_param_t *p){
    /* システムコールとサービスコールの処理関数の内部で
       ...
       サービスコールはthread_intrfec()内部の割り込みハンドラ呼び出しの
       延長で呼ばれているはずなので、呼び出し後にthread_intrvec()で
...
void kz_start(kz_func_t func, char *name, int priority, int stacksize, int argc, char *argv[]){
    ...
    thread_setintr(SOFTVEC_TYPE_SOFTERR, syscall_intr); /* ダウン要員発生 */

    /* システムコール発行負荷なので直接関数を呼び出してスレッド作成する。 */
